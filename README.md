...
## Installation on Raspbian 10 buster (nginx + php7.1)
...
1) Installs the necessary packages on your terminal:
    ```bash
    sudo apt install nginx php7.1-fpm php7.1-cli php7.1-mcrypt php7.1-sqlite3 git
    ```

2) Configure php:
modify the file /etc/php/7.1/fpm/php.ini, search and uncomment the line with cgi.fix_pathinfo and assigned the value zero:
    ```bash
    cgi.fix_pathinfo=0
    ```
...
5) Configure nginx with the configuration file present in piGardenWeb and personalize it if necessary:
    ...
    sudo cp piGardenWeb/virtualhost/nginx/default-php7.1 /etc/nginx/sites-available/default
    ```
